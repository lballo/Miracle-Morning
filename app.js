...
function AccueilScreen({ routines, affs, quals, onGoRoutines, onGoAffirmations }) {
  ...
  // Get dynamic affirmations/qualities by tag
  const getDynAffs = (tag) => {
    if (!tag) return [];
    return affs.filter(a => a.tags.includes(tag));
  };
  const getDynQuals = (tag) => {
    if (!tag) return [];
    return quals.filter(q => q.tags.includes(tag));
  };
  ...
  if (executing && !completed) {
    ...
    // Dynamic affirmations & qualities from user list by tag
    const dynAffs = getDynAffs(r.tag);
    const dynQuals = getDynQuals(r.tag);
      ...
      <div className="screen exec-screen">
        ...
        <div className="exec-inner">
          ...
          <div className="exec-body">
            ...
            {/* Hardcoded affirmation for this step */}
            {s.affirmation && (
              <div className="exec-aff-box">
                <div className="exec-section-label">AFFIRMATION</div>
                <div className="exec-aff-text">{s.affirmation}</div>
              </div>
            )}

            {/* Hardcoded qualities for this step */}
            {s.qualities && s.qualities.length > 0 && (
              <div className="exec-qual-box">
                <div className="exec-section-label">QUALITÉS</div>
                <div className="exec-qual-text">{s.qualities.join(' · ')}</div>
              </div>
            )}

            {/* Dynamic affirmations from user's list (matching tag) */}
            {s.showDynamic && dynAffs.length > 0 && (
              <div className="exec-aff-box dyn">
                <div className="exec-section-label">MES AFFIRMATIONS</div>
                {dynAffs.map(a => (
                  <div key={a.id} className="exec-dyn-aff">
                    « {a.text} » {a.fav && <span className="exec-fav-star">♥</span>}
                  </div>
                ))}
              </div>
            )}

            {/* Dynamic qualities from user's list (matching tag) */}
            {s.showDynamic && dynQuals.length > 0 && (
              <div className="exec-qual-box dyn">
                <div className="exec-section-label">MES QUALITÉS</div>
                <div className="exec-qual-text">
                  {dynQuals.map(q => q.name).join(' · ')}
                </div>
              </div>
            )}

            {/* Link to affirmations tab */}
            {(s.affirmation || s.showDynamic) && (
    ...
    <div className="screen" style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="screen-inner" style={{ position: 'relative', zIndex: 2 }}>
        <div style={{ paddingTop: 70, textAlign: 'center', marginBottom: 4 }}>
          <div className="title-serif" style={{ fontSize: 21 }}>Ta routine</div>
          <div className="small-label" style={{ marginTop: 6 }}>{bl} · {tot} min</div>
...
function JournalScreen() {
  ...
  const handleSave = () => {
    ...
    const entry = { id: Date.now(), date: `${d} ${m} ${y}`, text: text.trim(), q: '' };
    ...
    return (
      <div className="screen">
        ...
        <div className="screen-inner" style={{ position: 'relative', zIndex: 2 }}>
          ...
          {/* Note for custom routines: cannot launch */}
          {detail.custom && (
            <div style={{ textAlign: 'center', paddingBottom: 8 }}>
              <div style={{ fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 300, color: 'var(--text-soft)', opacity: 0.4, fontStyle: 'italic' }}>
                Les routines personnelles ne peuvent pas être lancées.{'\n'}Seuls les parcours pré-enregistrés sont disponibles à l'exécution.
              </div>
            </div>
          )}
        </div>
